...
func (s *server) handler(reg *prometheus.Registry) http.Handler {
	...
	r.Handle("QUERY", "/fizzbuzz", acceptQuery, s.query) // r.QUERY arrives in gin 1.13
...
func (s *server) fizzbuzz(c *gin.Context) {
	...
	setBodyHeaders(c, size)
	// A write error means the client left; net/http closes the connection.
	dw := &deadlineWriter{w: c.Writer, rc: http.NewResponseController(c.Writer), d: s.cfg.WriteChunkTimeout}
	_, _ = fizzbuzz.WriteJSON(dw, p)
}

// record never fails the request: a store outage costs statistics, not FizzBuzz.
// ponytail: synchronous, one Redis round trip per request; aggregate locally
// and flush in batches if that round trip ever shows in latency.
...
// logStoreErr logs at most once every 10 s, so an outage does not flood the log.
...
// parseParams requires each parameter exactly once and ignores unknown ones.
// Ranges are checked by Params.Validate.
...
func (d *deadlineWriter) Write(b []byte) (int, error) {
	_ = d.rc.SetWriteDeadline(time.Now().Add(d.d)) // unsupported under httptest
	return d.w.Write(b)
}
