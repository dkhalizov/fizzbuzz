...
// dcounter holds `,"<digits>"` right-aligned in d; digits grow left on carry.
...
func WriteJSON(w io.Writer, p Params) (int64, error) {
	pl := newPlan(p)
	// The periodic path buffers a whole period, so it is limited to periods
	// that fit the stream buffer.
...
// writePeriodic handles int1 == 1 or int2 == 1: the bytes repeat every
// P = max(a, b) elements, and a trailing partial period is a byte prefix of
// a full one because it contains no str1+str2.
func writePeriodic(w io.Writer, pl *plan) (int64, error) {
	P := min(max(pl.a, pl.b), pl.n)
	pb := int(pl.rangeBytes(1, P))
	k := min(streamBuf/pb, pl.n/P) // periods per chunk; WriteJSON calls this only when pb <= streamBuf
