...
func TestValidate(t *testing.T) {
	...
	// The suggested limit is exact: it fits, and one more does not.
	big := Params{Int1: 3, Int2: 5, Limit: DefaultLimits.MaxLimit, Str1: "fizz", Str2: strings.Repeat("b", 200)}
	n := largestLimit(big, DefaultLimits.MaxResponseBytes)
	if err := big.Validate(DefaultLimits); err == nil || !strings.Contains(err.Error(), fmt.Sprintf("use a limit of %d or less", n)) {
		t.Errorf("Validate = %v, want a suggested limit of %d", err, n)
	}
	for l, fits := range map[int]bool{n: true, n + 1: false} {
		big.Limit = l
		if got := JSONSize(big) <= DefaultLimits.MaxResponseBytes; got != fits {
			t.Errorf("limit %d: fits = %v, want %v", l, got, fits)
		}
	}
...
// The hand-optimized generator must match the literal reference for all
// input. go test also runs the seed corpus.
...
// One period of int1=1, int2=limit is the full response. The generator must
// still stream it through a bounded buffer, not build it in memory.
...
// TestDefaultLimits guards the reasons that the README gives for the defaults.
func TestDefaultLimits(t *testing.T) {
	lim := DefaultLimits
	classic := Params{Int1: 3, Int2: 5, Limit: lim.MaxLimit, Str1: "fizz", Str2: "buzz"}
	if JSONSize(classic) > lim.MaxResponseBytes {
		t.Error("the classic request does not fit at MaxLimit")
	}
	classic.Limit *= 10
	if JSONSize(classic) <= lim.MaxResponseBytes {
		t.Error("MaxLimit is not the largest power of ten at which the classic request fits")
	}
	const v8MaxString = 1<<29 - 24 // characters in one JavaScript string: 512 MiB - 24
	if lim.MaxResponseBytes > (v8MaxString+24)/2 {
		t.Error("MaxResponseBytes is more than half of the 512 MiB V8 string limit")
	}
	// Worst case for short strings: every element is "str1str2".
	worst := Params{Int1: 1, Int2: 1, Limit: lim.MaxLimit, Str1: "x", Str2: strings.Repeat("x", guaranteedStrBytes-1)}
	if err := worst.Validate(lim); err != nil {
		t.Errorf("%d bytes of strings: %v", guaranteedStrBytes, err)
	}
	worst.Str2 += "x"
	if worst.Validate(lim) == nil {
		t.Errorf("%d bytes of strings fit: the guarantee is not the largest", guaranteedStrBytes+1)
	}
}

// guaranteedStrBytes is the README guarantee: with the defaults, str1 and str2
// of this many bytes together, after JSON escaping, fit at every limit.
const guaranteedStrBytes = 23

// FuzzSizeLimit checks the size rule for any combination of parameters: the
// suggested limit is exact, and short strings always fit.
func FuzzSizeLimit(f *testing.F) {
	f.Add(uint64(3), uint64(5), uint32(10_000_000), "fizz", "buzz")
	f.Add(uint64(1), uint64(1), uint32(10_000_000), "x", strings.Repeat("y", 300))
	f.Add(uint64(2), uint64(3), uint32(9_999_999), "\x00<", "é\"")
	f.Fuzz(func(t *testing.T, a, b uint64, n uint32, s1, s2 string) {
		lim := DefaultLimits
		p := Params{Int1: int(a%math.MaxInt) + 1, Int2: int(b%math.MaxInt) + 1, Limit: int(n)%lim.MaxLimit + 1, Str1: s1, Str2: s2}
		big := lim
		big.MaxResponseBytes = math.MaxInt64
		if p.Validate(big) != nil {
			return // A field is out of range. TestValidate covers that.
		}
		escaped := func(s string) int { b, _ := json.Marshal(s); return len(b) - 2 }
		err := p.Validate(lim)
		if fits := JSONSize(p) <= lim.MaxResponseBytes; fits != (err == nil) {
			t.Fatalf("Validate = %v, but JSONSize fits = %v", err, fits)
		}
		if escaped(s1)+escaped(s2) <= guaranteedStrBytes && err != nil {
			t.Fatalf("short strings rejected: %v", err)
		}
		if err == nil {
			return
		}
		m := largestLimit(p, lim.MaxResponseBytes)
		q := p
		for l, want := range map[int]bool{m: true, m + 1: false} {
			if q.Limit = l; l >= 1 && (JSONSize(q) <= lim.MaxResponseBytes) != want {
				t.Fatalf("limit %d: fits != %v", l, want)
			}
		}
		if m > 0 && !strings.Contains(err.Error(), fmt.Sprintf("use a limit of %d or less", m)) {
