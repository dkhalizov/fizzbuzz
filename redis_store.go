...
// Atomic across replicas: a new member is admitted only if it fits the byte
// budget, otherwise the store saturates for good. Returns 0 counted,
// 1 already saturated, 2 saturated by this call.
...
// redisStore shares exact counts between replicas. Saturation lasts until the
// keys are deleted, not until restart.
