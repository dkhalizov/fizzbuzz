...
| `compose.yaml` | The service with Redis on one machine |
| `loadtest/` | k6 scripts for a deployed instance |
...
- The output of `internal/fizzbuzz` must be byte-identical to `json.Marshal` of the literal `reference` in its tests. After a change there, run both fuzz targets.
...
- Put each new setting in `config.go` and in the README configuration table in the same commit. Give each request limit a reason in "Why these limits". If the reason is a fact that can change, add a test that fails when it stops being true.
