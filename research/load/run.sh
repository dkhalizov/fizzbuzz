...
S=$(cd "$(dirname "$0")" && pwd)
PORT=18080
# LOGSINK=pipe sends stdout through a pipe to a reader on the server cores,
# as a container runtime does. The default is /dev/null.
if [ "$LOGSINK" = pipe ]; then
  FIFO=$(mktemp -u); mkfifo $FIFO
  taskset -c $SC cat $FIFO > /dev/null &
  GOMAXPROCS=$GMP PORT=$PORT taskset -c $SC $BIN > $FIFO 2>&1 &
  PID=$!
else
...
kill $PID; wait $PID 2>/dev/null; [ -n "$FIFO" ] && rm -f $FIFO
