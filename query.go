...
// QUERY (RFC 10008) is GET with the input in the body: safe, idempotent,
// counted in /stats like GET. The body is the same form encoding as the GET
// query string, so both share one parser and one set of rules.
...
func (s *server) query(c *gin.Context) {
	// RFC 10008 §2: a missing or unsupported Content-Type must fail.
	...
	// The same budget the query string gets as a request header.
