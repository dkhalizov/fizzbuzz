...
async function send() {
  ...
  const qs = q.toString();
  const run = { ctrl: new AbortController(), got: 0, total: 0, t0: performance.now(), tHead: 0, done: false };
  current = run;
  ...
  try {
    ...
    run.total = Number(res.headers.get('Content-Length')) || 0;
...
function paint(run) {
  draw();
  const pct = run.total ? Math.min(1, run.got / run.total) : 0;
  const bar = $('fill');
  bar.style.width = pct * 100 + '%';
  bar.classList.toggle('done', run.done && run.got === run.total);
  $('f-len').textContent = run.total ? nf.format(run.total) + ' B' : '–';
  ...
  if (!run.done && run.total) status(`${Math.floor(pct * 100)}% received`, '');
}

function finish(run) {
  const m = map;
  const exact = run.got === run.total;
  ...
  if (!exact) {
    status(`Received ${nf.format(run.got)} bytes, but Content-Length is ${nf.format(run.total)}.`, 'bad');
  } else if (!complete || m.wrong || m.counts[4]) {
    status(`Received ${nf.format(run.total)} bytes, which matches Content-Length. ${nf.format(m.wrong + (m.n - m.count))} elements do not match the rule.`, 'bad');
  } else {
    status(`Received exactly ${nf.format(run.total)} bytes, which matches Content-Length. ${scope} match the rule.`, 'ok');
  }
}
...
function showExchange(method, qs, res) {
  ...
  for (const h of ['Content-Type', 'Content-Length', 'Accept-Query', 'Allow', 'Retry-After']) {
